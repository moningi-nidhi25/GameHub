...
function updateGame() {
  ...
  const lengthProgress = document.getElementById("lengthProgress");
  if (lengthProgress) lengthProgress.style.width = `${Math.min(snake.length * 5, 100)}%`;

  const foodEl = document.getElementById("foodEaten");
  if (foodEl) foodEl.textContent = foodEaten;
